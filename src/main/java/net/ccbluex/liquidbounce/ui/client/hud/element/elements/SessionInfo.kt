...
import net.ccbluex.liquidbounce.features.module.modules.client.BanChecker
...
class SessionInfo(x: Double = 10.0, y: Double = 10.0, scale: Float = 1F) : Element(x, y, scale) {
    ...
    private val blurStrength = IntegerValue("BlurStrength", 10, 1, 60)
    ...
    override fun drawElement(partialTicks: Float): Border {
        ...
        if (modeValue.get().equals("1")) {
            for (i in 0..(gradientAmountValue.get() - 1)) {
                ...
                Fonts.fontSFUI35.drawString(
                    "${StatisticsUtils.getKills()}",
                    165 - Fonts.fontSFUI35.getStringWidth("${StatisticsUtils.getKills()}") - 3F,
                    27F,
                    Color(0xFFFFFF).rgb
                )
        ...
        if (modeValue.get().equals("3")) {
            ...
            var durationInMillis: Long = System.currentTimeMillis() - SessionUtils.getFormatSessionTime().toInt()
