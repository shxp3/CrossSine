...
class FloatElement(val savedValue: FloatValue): ValueElement<Float>(savedValue) {
    ...
    override fun drawElement(mouseX: Int, mouseY: Int, x: Float, y: Float, width: Float, bgColor: Color, accentColor: Color): Float {
        val valueDisplay = 30F + Fonts.SFApple40.getStringWidth("${savedValue.maximum.toInt().toFloat() + 0.01F}")
        val maxLength = Fonts.SFApple40.getStringWidth("${savedValue.maximum}")
        val minLength = Fonts.SFApple40.getStringWidth("${savedValue.minimum}")
        val nameLength = Fonts.SFApple40.getStringWidth(value.name)
        ...
        val currLength = Fonts.SFApple40.getStringWidth("${round(savedValue.get())}")
        Fonts.SFApple40.drawString(value.name, x + 10F, y + 10F - Fonts.SFApple40.FONT_HEIGHT / 2F + 2F, -1)
        Fonts.SFApple40.drawString("${savedValue.maximum}",
                                x + width - 10F - maxLength - valueDisplay, 
                                y + 10F - Fonts.SFApple40.FONT_HEIGHT / 2F + 2F, -1)
        Fonts.SFApple40.drawString("${savedValue.minimum}",
        ...
        Fonts.SFApple40.drawString("${round(savedValue.get())}", x + width + 6F - valueDisplay, y + 10F - Fonts.SFApple40.FONT_HEIGHT / 2F + 2F, -1)
        Fonts.SFApple40.drawString("-", x + width - 3F - valueDisplay, y + 10F - Fonts.SFApple40.FONT_HEIGHT / 2F + 2F, -1)
        Fonts.SFApple40.drawString("+", x + width - 17F, y + 10F - Fonts.SFApple40.FONT_HEIGHT / 2F + 2F, -1)
