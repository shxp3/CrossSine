...
class GuiBackground(val prevGui: GuiScreen) : GuiScreen() {
    ...
    private fun updateButtons() {
        enabledButton.displayString = "Status (${if (enabled) "ON" else "OOF"})"
        blurButton.displayString = "Background Blur (${if (blur) "ON" else "OOF"})"
        typeButton.displayString = "Background: ${GradientBackground.gradientSide}"
        particlesButton.displayString = "Particle (${if (particles) "ON" else "OOF"})"
        animatedButton.displayString = "Animetion (${if (GradientBackground.animated) "ON" else "OOF"})"
        val hasCustomBackground = CrossSine.background != null
    ...
    override fun actionPerformed(button: GuiButton) {
        when (button.id) {
            ...
            3 -> {
                ...
                try {
                    Files.copy(file.toPath(), FileOutputStream(CrossSine.fileManager.backgroundFile))

                    val image = ImageIO.read(FileInputStream(CrossSine.fileManager.backgroundFile))
                    CrossSine.background = ResourceLocation(CrossSine.CLIENT_NAME.lowercase() + "/background.png")
                    mc.textureManager.loadTexture(CrossSine.background, DynamicTexture(image))
                } catch (e: Exception) {
                    e.printStackTrace()
                    MiscUtils.showErrorPopup("Error", "Exception class: " + e.javaClass.name + "\nMessage: " + e.message)
                    CrossSine.fileManager.backgroundFile.delete()
                }
            }
            4 -> {
                CrossSine.background = null
                CrossSine.fileManager.backgroundFile.delete()
            }
            5 -> {
    ...
    override fun drawScreen(mouseX: Int, mouseY: Int, partialTicks: Float) {
        ...
        mc.fontRendererObj.drawCenteredString("Current Backgrond: " + if(CrossSine.background == null) { GradientBackground.nowGradient.name } else { "Customized" },
