...
open class Targets : Element(-46.0, -40.0, 1F, Side(Side.Horizontal.MIDDLE, Side.Vertical.MIDDLE)) {
    ...
    private val colorModeValue = ListValue("Color", arrayOf("Custom", "Rainbow", "Sky", "Slowly", "Fade", "mixer", "Health"), "Health")
    ...
    private val RiseredValue = IntegerValue("RiseRed", 252, 0, 255)
    private val RisegreenValue = IntegerValue("RiseGreen", 96, 0, 255)
    private val RiseblueValue = IntegerValue("RiseBlue", 66, 0, 255)
    ...
    private fun drawNovo3(target: EntityLivingBase) {
        ...
        if (mc.netHandler.getPlayerInfo(target.uniqueID) != null) drawHead(mc.netHandler.getPlayerInfo(target.uniqueID).locationSkin, 0, 0, 36, 36)
    ...
    private fun drawAsuid(entity: EntityPlayer) {
    ...
    private fun drawRiseLatest(target: EntityLivingBase) {
        ...
        font.drawString("Name: ${target.name}", 45, 14, Color(RiseredValue.get(), RisegreenValue.get(), RiseblueValue.get(), 255).rgb)
        ...
        RenderUtils.drawRoundedCornerRect(50f, 31f, 50f + (healthBarWidth * (easingHP / target.maxHealth)) , 39f, 4f, Color(RiseredValue.get(), RisegreenValue.get(), RiseblueValue.get(), 255).rgb)
        ...
        font.drawString(getHealth(target).roundToInt().toString(), ((38 + additionalWidth.toInt() - font.getStringWidth((getHealth(target) * 1.15).roundToInt().toString()).toInt()) / 1.15).roundToInt()   , 31 - (font.FONT_HEIGHT/2).toInt(), Color(RiseredValue.get(), RisegreenValue.get(), RiseblueValue.get(), 255).rgb)
        GL11.glPopMatrix()
    ...
    private fun drawHanabi(entity: EntityLivingBase) {
        updateAnim(entity.health)
        ...
        healthBarWidth = RenderUtils.getAnimationStateSmooth(
            hpWidth,
            healthBarWidth,
            (14f / Minecraft.getDebugFPS()).toDouble()
        ).toFloat().toDouble()
        hudHeight =
            RenderUtils.getAnimationStateSmooth(40.0, hudHeight, (8f / Minecraft.getDebugFPS()).toDouble())
        if (hudHeight == 0.0) {
        ...
        RenderUtils.drawGradientSideways(
            ...
            40.0, barColor.rgb, barColor.rgb
        )
        easingHealth += ((entity.health - easingHealth) / 2.0F.pow(10.0F - fadeSpeed.get())) * RenderUtils.deltaTime
    ...
    private fun drawFlux2(target: EntityLivingBase) {
        ...
        if (mc.netHandler.getPlayerInfo(target.uniqueID) != null) drawHead(mc.netHandler.getPlayerInfo(target.uniqueID).locationSkin, 1, 1, 26, 26)
        Fonts.fontSFUI40.drawString(target.name, 30F, 5F, 0xFFFFFF) // Draw convertedTarget name
        Fonts.fontSFUI35.drawString("Health: ${decimalFormat2.format(target.health)}", 30F, 17.5F, 0xFFFFFF) // Draw convertedTarget health
    ...
    private fun drawMoonOld(entity: EntityPlayer) {
        updateAnim(entity.health)
        ...
        RenderUtils.newDrawRect(53F, 41f, 17.5F + (easingHealth / entity.maxHealth).coerceIn(0F, 1F) * 138F, 48.5F, barColor.rgb)
        ...
        Fonts.fontSFUI35.drawString("Health: ${(decimalFormat.format(entity.health))}", 53.5f, 22f, barColor.rgb)
        // Draw info
