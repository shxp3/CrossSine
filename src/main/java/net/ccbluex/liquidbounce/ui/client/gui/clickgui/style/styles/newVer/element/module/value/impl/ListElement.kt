...
class ListElement(val saveValue: ListValue): ValueElement<String>(saveValue) {
    ...
    private val maxSubWidth = -(saveValue.values.map { -Fonts.Nunito40.getStringWidth(it) }.sorted().firstOrNull() ?: 0F).toFloat() + 20F
    ...
    override fun drawElement(mouseX: Int, mouseY: Int, x: Float, y: Float, width: Float, bgColor: Color, accentColor: Color): Float {
        ...
        glPopMatrix()
        glPopMatrix()
        Fonts.Nunito40.drawString(value.get(), x + width - 14F - maxSubWidth, y + 6F, -1)
        glPushMatrix()
        ...
        if (percent > 0F) for (subV in unusedValues) {
            Fonts.Nunito40.drawString(subV, 0F, (16F + vertHeight) * percent - 1F, Color(.5F, .5F, .5F, percent.coerceIn(0F, 1F)).rgb)
            vertHeight += 16F
        }
        glPopMatrix()
