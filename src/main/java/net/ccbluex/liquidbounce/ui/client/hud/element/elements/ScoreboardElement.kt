...
class ScoreboardElement(
    ...
    private val roundStrength = FloatValue("Rounded-Strength", 5F, 0F, 10F).displayable { bgRoundedValue.get() }
    ...
    private val themeBarValue = BoolValue("ColorThemeBat", false)
    private val shadowValue = BoolValue("ShadowText", false)
    private val serverValue = BoolValue("CustomIp", false)
    ...
    override fun drawElement(partialTicks: Float): Border? {
        ...
        for (score in scoreCollection) {
            ...
            if (ServerUtils.isHypixelDomain(stripped)) {
                name = if (serverValue.get()) CrossSine.CUSTOM_DOMAIN else ""
                listColor = getColor(1).rgb
                break
            }
        }
        ...
        if (themeBarValue.get()) {
            Gui.drawRect(l1 - 7, -6, 9, -5, ClientTheme.getColor(1).rgb)
        }
        // draw main rect?
        if (!bgRoundedValue.get()) {
            Gui.drawRect(l1 - 7, -5, 9, maxHeight + fontRenderer.FONT_HEIGHT + 5, backColor)
        } else {
            RenderUtils.drawRoundedRect(l1 - 7F, -5F, 9F, maxHeight + fontRenderer.FONT_HEIGHT + 5F, roundStrength.get() ,backColor)
        }
        ...
        scoreCollection.forEachIndexed { index, score ->
            ...
            if (!serverValue.equals("none")) {
                for (domain in allowedDomains) {
                    if (name.contains(domain, true)) {
                        name = if (serverValue.get()) CrossSine.CUSTOM_DOMAIN else ""
                        listColor =  getColor(index + 1).rgb
                        break
                    }
                }
            }
            ...
            if (rectValue.get()) {
                val rectColor = when {
                    rectColorMode.equals("Theme", ignoreCase = true) -> ClientTheme.getColor(index).rgb
