...
class ScoreboardElement(
    ...
    var mixerColor: Int = ColorMixer.getMixedColor(  HUD.mixerDistValue.get() * 10, HUD.mixerSecValue.get()).rgb
    ...
    override fun drawElement(partialTicks: Float): Border? {
        ...
        for (score in scoreCollection) {
            ...
            if (ServerUtils.isHypixelDomain(stripped)) {
                ...
                listColor = when (HUD.ClientColorMode.get().lowercase()) {
                    "rainbow" ->  ColorUtils.slowlyRainbow(System.nanoTime(),  30 * 1, 1F, 1F).rgb
                    "astolfo" -> ColorUtils.astolfo( 1, indexOffset = 100 * 2).rgb
                    "mixer" -> mixerColor
                    "fade" -> ColorUtils.fade(Color(HUD.ColorRed.get(),HUD.ColorGreen.get(),HUD.ColorBlue.get()),  HUD.fadeDistanceValue.get(), 100).rgb
                    else -> Color(HUD.ColorRed.get(),HUD.ColorGreen.get(),HUD.ColorBlue.get()).rgb
                }
                break
            }
        }
