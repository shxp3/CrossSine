...
@ModuleInfo(name = "Projectiles", spacedName = "Projectiles", category = ModuleCategory.VISUAL)
class Projectiles : Module() {
    ...
    fun onRender3D(event: Render3DEvent) {
        ...
        RenderUtils.glColor(if (hitEntity) { Color(textColor().rgb) } else { Color(textColor().rgb) })
        GL11.glLineWidth(2f)
        ...
        if (landingPosition != null) {
            ...
            RenderUtils.drawAxisAlignedBB(AxisAlignedBB(-0.5, 0.0, -0.5, 0.5, 0.1, 0.5), if (hitEntity) { Color(textColor().rgb) } else { Color(textColor().rgb) }, true, true, 3f)
        }
        GL11.glPopMatrix()
