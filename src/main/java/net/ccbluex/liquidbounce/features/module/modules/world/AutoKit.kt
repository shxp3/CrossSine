...
@ModuleInfo(name = "AutoKit", spacedName = "AutoKit", category = ModuleCategory.WORLD)
class AutoKit : Module() {
    ...
    private val editMode: BoolValue = object : BoolValue("Edit-Mode", false) {
        override fun onChanged(oldValue: Boolean, newValue: Boolean) {
            if (newValue)
                CrossSine.hud.addNotification(Notification("INFO","Change default kit by right clicking the kit selector and select.", NotifyType.INFO))
        }
    }
    ...
    fun onUpdate(event: UpdateEvent) {
        ...
        if (clickStage == 2) {
            ...
            if (timeoutTimer.hasTimePassed(kitTimeOutValue.get())) {
                ...
                CrossSine.hud.addNotification(Notification("ERROR", "Kit checker timed out. Please use the right kit name.", NotifyType.ERROR))
    ...
    fun onPacket(event: PacketEvent) {
        ...
        if (packet is C0DPacketCloseWindow && editMode.get()) {
            editMode.set(false)
            CrossSine.hud.addNotification(Notification("INFO","Edit mode aborted.", NotifyType.INFO))
        ...
        if (packet is S02PacketChat) {
            ...
            if (text.contains("kit has been selected", true)) {
                if (editMode.get()) {
                    ...
                    clickStage = 0
                    listening = false
                    CrossSine.hud.addNotification(Notification("SUCCESS","Successfully detected and added $kitName kit.", NotifyType.SUCCESS))
                    debug("finished detecting kit")
                    return
                } else {
                    listening = false
                    event.cancelEvent()
                    CrossSine.hud.addNotification(Notification("SUCCESS","Successfully selected ${kitNameValue.get()} kit.", NotifyType.SUCCESS))
                    debug("finished")
                    return
                }
            }
