...
@ModuleInfo(name = "KillESP", spacedName = "Kill ESP", category = ModuleCategory.VISUAL)
class KillESP : Module() {
    ...
    override fun onInitialize() {
        aura = CrossSine.moduleManager.getModule(KillAura::class.java)
    }
    ...
    fun onRender3D(event: Render3DEvent?) {
        if (modeValue.get().equals("jello", ignoreCase = true) && !aura!!.targetModeValue.get()
            ...
            if (!aura!!.targetModeValue.get().equals("multi", ignoreCase = true) && aura!!.currentTarget != null) {
                val tracers = CrossSine.moduleManager.getModule(Tracers::class.java) ?: return
