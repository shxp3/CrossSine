...
@ModuleInfo(name = "XRay", spacedName = "XRay", category = ModuleCategory.VISUAL, autoDisable = EnumAutoDisableType.RESPAWN, moduleCommand = false)
class XRay : Module() {
    ...
    init {
        CrossSine.commandManager.registerCommand(object : Command("xray", emptyArray()) {
                    ...
                    if (args[1].equals("add", ignoreCase = true)) {
                        if (args.size > 2) {
                            try {
                                ...
                                CrossSine.fileManager.saveConfig(CrossSine.fileManager.xrayConfig)
