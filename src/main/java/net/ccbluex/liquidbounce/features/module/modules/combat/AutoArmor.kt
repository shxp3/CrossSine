...
import net.ccbluex.liquidbounce.utils.AutoArmor.ArmorComparator
import net.ccbluex.liquidbounce.utils.AutoArmor.ArmorPiecee
...
@ModuleInfo(name = "AutoArmor", "Auto Armor", category = ModuleCategory.COMBAT)
class AutoArmor : Module() {
    ...
    fun onRender3D(event: Render3DEvent?) {
        ...
        val armorPieces = IntStream.range(0, 36)
            ...
            .mapToObj { i: Int -> ArmorPiecee(mc.thePlayer.inventory.getStackInSlot(i), i) }
            .collect(
                Collectors.groupingBy(
                    Function { obj: ArmorPiecee? -> obj!!.armorType })
            )
        val bestArmor = arrayOfNulls<ArmorPiecee>(4)
        ...
        for (i in 0..3) {
            ...
            val armorSlot = 3 - i
            val oldArmor = ArmorPiecee(mc.thePlayer.inventory.armorItemInSlot(armorSlot), -1)
    ...
    private fun move(item: Int, isArmorSlot: Boolean): Boolean {
        if (!isArmorSlot && item < 9 && hotbarValue.get() && mc.currentScreen !is GuiInventory) {
            ...
            val openInventory = invSpoof.get() && mc.currentScreen !is GuiInventory
