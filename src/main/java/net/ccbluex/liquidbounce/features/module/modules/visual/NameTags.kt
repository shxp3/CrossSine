...
@ModuleInfo(name = "NameTags", spacedName = "Name Tags", category = ModuleCategory.VISUAL)
class NameTags : Module() {
    ...
    private fun renderNameTag(entity: EntityLivingBase, tag: String) {
        if (onlyTarget.get() && entity != CrossSine.combatManager.target && entity.getName() != entityKeep) {
            return
        } else if (onlyTarget.get() && entity == CrossSine.combatManager.target) {
            entityKeep = entity.getName()
            targetTicks++
            if (targetTicks >= 5) {
                targetTicks = 4
            }
        } else if (onlyTarget.get() && CrossSine.combatManager.target == null) {
            targetTicks--
            if (targetTicks <= -1) {
                targetTicks = 0
                entityKeep = "zywl gg"
