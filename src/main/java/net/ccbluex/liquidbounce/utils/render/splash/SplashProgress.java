package net.ccbluex.liquidbounce.utils.render.splash;
...
public class SplashProgress {
    ...
    private static SplashProgress.Texture backgroundTexture;
    ...
    public static void start() {
        ...
        if (enabled) {
            ...
            thread = new Thread(new Runnable() {
                ...
                public void run() {
                    this.setGL();
                    SplashProgress.fontTexture = new SplashProgress.Texture(fontLoc);
                    //SplashProgress.logoTexture = new SplashProgress.Texture(logoLoc);
                    SplashProgress.backgroundTexture = new SplashProgress.Texture(backgroundLoc);
                    //SplashProgress.forgeTexture = new SplashProgress.Texture(forgeLoc);
                    GL11.glEnable(3553);
                    SplashProgress.fontRenderer = new SplashProgress.SplashFontRenderer();
                    GL11.glDisable(3553);

                    for(; !SplashProgress.done; Display.sync(100)) {
                        ...
                        GL11.glEnable(3553);
                        SplashProgress.backgroundTexture.bind();
                        GL11.glBegin(7);
                        SplashProgress.backgroundTexture.texCoord(0, 0.0F, 0.0F);
                        GL11.glVertex2f((-w2 / 2) + startX, (-h2 / 2) + startY);
                        SplashProgress.backgroundTexture.texCoord(0, 0.0F, 1F);
                        GL11.glVertex2f((-w2 / 2) + startX, (h2 / 2) + startY);
                        SplashProgress.backgroundTexture.texCoord(0, 1F, 1F);
                        GL11.glVertex2f((w2 / 2) + startX, (h2 / 2) + startY);
                        SplashProgress.backgroundTexture.texCoord(0, 1F, 0.0F);
                        GL11.glVertex2f((w2 / 2) + startX, (-h2 / 2) + startY);
                        GL11.glEnd();
                        GL11.glDisable(3553);
    ...
    private static class SplashFontRenderer extends FontRenderer {
        public SplashFontRenderer() {
            super(Minecraft.getMinecraft().gameSettings, SplashProgress.fontTexture.getLocation(), null, false);
    ...
    private static class Texture {
        ...
        public Texture(ResourceLocation location) {
            ...
            try {
                ...
                s = SplashProgress.open(location);
                ...
                if (!readers.hasNext()) {
                    ...
                    this.size = size;
                    GL11.glEnable(3553);
                    Class var8 = SplashProgress.class;
                    ...
                    for(int i = 0; i * (size / this.width) < this.frames; ++i) {
                        for(int j = 0; i * (size / this.width) + j < this.frames && j < size / this.width; ++j) {
                            ...
                            GL11.glTexSubImage2D(3553, 0, j * this.width, i * this.height, this.width, this.height, 32993, 33639, SplashProgress.buf);
