...
public class ArmorComparator implements Comparator<ArmorPiecee> {
    ...
    @Override
    public int compare(ArmorPiecee o1, ArmorPiecee o2) {
        ...
        if (compare == 0) {
            ...
            if (otherEnchantmentCmp == 0) {
                int enchantmentCountCmp = Integer.compare(ItemUtil.getEnchantmentCount(o1.getItemStack()), ItemUtil.getEnchantmentCount(o2.getItemStack()));
    ...
    private float getThresholdedEnchantmentDamageReduction(ItemStack itemStack) {
        ...
        for (int i = 0; i < DAMAGE_REDUCTION_ENCHANTMENTS.length; i++) {
            sum += ItemUtil.getEnchantment(itemStack, DAMAGE_REDUCTION_ENCHANTMENTS[i]) * ENCHANTMENT_FACTORS[i] * ENCHANTMENT_DAMAGE_REDUCTION_FACTOR[i];
        }
    ...
    private float getEnchantmentThreshold(ItemStack itemStack) {
        ...
        for (int i = 0; i < OTHER_ENCHANTMENTS.length; i++) {
            sum += ItemUtil.getEnchantment(itemStack, OTHER_ENCHANTMENTS[i]) * OTHER_ENCHANTMENT_FACTORS[i];
        }
